...
interface RemoteEntity {
  bot: Bot;
  buffer: RemoteSample[];
  /** Ботов ведёт сервер, поэтому для совпадения коллизий рисуем их без сетевого отставания. */
  isBot: boolean;
}
...
export class CityRideGame {
  ...
  applyEntities(snapshot: EntitySnapshot): void {
    ...
    this.syncRemoteEntities(others, t, new Set(snapshot.bots.map((bot) => bot.id)));
  }
   ...
   */
  private syncRemoteEntities(
    entities: RemoteEntityState[],
    t: number,
    ...
    for (const entity of entities) {
      ...
      const remote: RemoteEntity = known ?? {
        bot: this.toRenderBot(entity),
        buffer: [],
        isBot: botIds.has(entity.id),
      };
      remote.isBot = botIds.has(entity.id);
  ...
  private updateRemoteEntities(dt: number): void {
    ...
    for (const entity of this.remoteEntities.values()) {
      // Интерполяционный буфер специально рисует сетевые сущности в прошлом.
      // Для быстро едущего бота это уводило видимый кузов на десятки пикселей
      // от позиции, где авторитетный сервер считает столкновение с игроком.
      // Бота продлеваем по уже известным скорости и повороту до текущего момента;
      // живых игроков оставляем на плавной буферизованной шкале.
      const at = sampleTimeline(
        entity.buffer,
        render + (entity.isBot ? this.timeline.delay : 0)
      );
      if (!at) continue;
      entity.bot.x = at.x;
      entity.bot.y = at.y;
  ...
  applyWorldObjects(objects: WorldObjects): void {
    const previousStations = new Map(
      this.city.stations.flatMap((station) => (station.id ? [[station.id, station] as const] : []))
    );
    for (const station of objects.stations) {
      const previous = station.id ? previousStations.get(station.id) : undefined;
      if (previous?.state === "active" && station.state === "locked") {
        this.killPointer(station.x + station.w / 2, station.y + station.h / 2, FUEL_ICON_PATH);
      }
    }
  ...
  private updateDeadPointers(dt: number): void {
